...
def model_predict_spbi(**kwargs):
    ...
    s3_hook.download_file(
        key = "catboost_spb.joblib",
        bucket_name = "studcamp-ml",
        local_path = 'data',
        preserve_file_name = True,
        use_autogenerated_subdir = False
    )
    model = joblib.dump("catboost_spb.joblib")
...
def save_last_hundred_imoex(**kwargs):
    ...
    s3_hook.load_file(last_hundred, key = "last_hundred_imoex.csv", 
                        bucket_name="studcamp-ml", replace=True)
    remove('data/IMOEX_filled.csv')
...
def save_last_hundred_spbi(**kwargs):
    ...
    s3_hook.load_file(last_hundred, key = "last_hundred_spbi.csv", 
                        bucket_name="studcamp-ml", replace=True)
    remove('data/SPBIRUS2_filled.csv')
...
def retrain_model_imoex(**kwargs):
    ti = kwargs["ti"]
    info = ti.xcom_pull(task_ids="save_last_hundred_imoex")
    ...
    df = pd.read_csv('data/for_catboost.csv')
    s3_hook.download_file(
...
def retrain_model_spbirus(**kwargs):
    ti = kwargs["ti"]
    info = ti.xcom_pull(task_ids="retraining_imoex")
    info["retraining_imoex_start"] = datetime.now().strftime("%Y%m%d %H:%M") 
    ...
    q = pd.read_csv('data/yesterday_query.csv')
    df = pd.concat([df, q], reset_index = False)
    cbr = cbr(**params)
    cbr.fit(df)
    model = joblib.dump("catboost.joblib")
